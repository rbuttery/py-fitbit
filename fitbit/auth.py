...
class FitbitOauth2Client(object):
    ...
    def authorize_token_url(self, scope=None, redirect_uri=None, **kwargs):
        ...
        self.session.scope = scope or [ # all scopes
            "activity",
            "nutrition",
            "heartrate",
            "location",
    ...
    def get_local_tokens(self) -> Tuple[str, str] | None:
        try:
            ...
            try:
                # run auth server
                from fitbit.auth_server import OAuth2Server
                ...
                print(f"Error getting local tokens: {e}")
            return None
